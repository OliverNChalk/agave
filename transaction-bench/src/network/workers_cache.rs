...
use {
    ...
    crate::generator::TransactionBatch,
    log::*,
    lru::LruCache,
    std::{
        collections::HashMap,
...
impl WorkersCache {
    ...
    pub async fn push(&mut self, peer: SocketAddr, peer_worker: WorkerInfo) {
        // Although there might be concerns about the performance implications of waiting for the
        // worker to be closed when trying to add a new worker, the idea is that these workers are
        // almost always created in advance so the latency is hidden.
        ...
        let body = async move {
            ...
            if let Err(WorkersCacheError::ReceiverDropped) = send_res {
                ...
                if let Some(current_worker) = workers.pop(peer) {
                    // To avoid obscuring the error from send, ignore a possible `TaskJoinFailure`.
