...
use {
    crate::{
        accounts_file::{write_accounts_file, AccountsFile},
        cli::AccountParams,
        range::Range,
    },
    chrono::prelude::Utc,
...
impl AccountsCreator {
    ...
    pub async fn create(&self) -> Result<AccountsFile, AccountsCreatorError> {
        ...
        if payers.len() != self.account_params.num_payers {
            error!(
            ...
            if !payers.is_empty() {
                save_partial_results(payers, vec![]);
            }
        ...
        if sized_accounts.len() != self.account_params.num_accounts {
            if sized_accounts.is_empty() {
                error!("Failed to create sized accounts.");

                return Err(AccountsCreatorError::CreateAccountFailure);
            }
            save_partial_results(payers, sized_accounts);
            return Err(AccountsCreatorError::CreateAccountFailure);
        }
...
fn save_partial_results(payers: Vec<Keypair>, sized_accounts: Vec<Keypair>) {
    ...
    let accounts = AccountsFile {
        owner_program_id: Pubkey::new_unique(),
        payers,
        sized_accounts,
    };
