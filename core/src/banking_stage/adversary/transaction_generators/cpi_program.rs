//! Creates a generator that executes the program, which makes serveral CPI calls (some recursively)
...
pub fn verify(accounts: &AccountsFile, attack: &Attack) -> Result<(), String> {
    let Attack::CpiProgram(attack) = attack else {
        panic!("Unexpected Attack passed into `cpi_program::verify`: {attack:?}",);
    };
...
fn create_cpi_message(
    payer: &Keypair,
    program_id: &Pubkey,
    accounts_meta: &[AccountMeta],
    transaction_cu_budget: u32,
) -> Message {
    let data = BlockGeneratorStressTestInstruction::Cpi {
        depth: MAX_CPI_DEPTH,
    ...
    let cpi_instruction = Instruction::new_with_borsh(*program_id, &data, accounts_meta);
    Message::new(
        &[set_cu_instruction, cpi_instruction],
        Some(&payer.pubkey()),
    )
}
...
mod tests {
    ...
    fn rpc_config_valid() {
        ...
        let config = AdversarialConfig {
            selected_attack: Some(Attack::CpiProgram(AttackProgramConfig {
