...
if [ -n "$ATTACK_TARGET" ]; then
  REPAIR_SH_ARGS+=("--attack-target" "$ATTACK_TARGET")
fi
...
commands=(
  "$BIN ${COMMON_ARGS[@]} configure-invalidate-leader-block --invalidation-kind invalidFeePayer"
  "sleep $RUNTIME"
  "$BIN ${COMMON_ARGS[@]} configure-invalidate-leader-block"
  "sleep $SLEEPTIME"
  "$BIN ${COMMON_ARGS[@]} configure-invalidate-leader-block --invalidation-kind invalidSignature"
  "sleep $RUNTIME"
  "$BIN ${COMMON_ARGS[@]} configure-invalidate-leader-block"
  "sleep $SLEEPTIME"
  "$BIN ${COMMON_ARGS[@]} configure-drop-turbine-votes --drop true"
  "sleep $RUNTIME"
  "$BIN ${COMMON_ARGS[@]} configure-drop-turbine-votes --drop false"
  "sleep $SLEEPTIME"
  ...
  "$SCRIPT_DIR/repair-tests.sh ${REPAIR_SH_ARGS[@]} --test disable"
  "sleep $SLEEPTIME"
  "$SCRIPT_DIR/repair-tests.sh ${REPAIR_SH_ARGS[@]} --test fake_future_leader_slots --iteration-delay-us 10000000 --packets-per-iteration 20"
  "sleep $RUNTIME"
  "$SCRIPT_DIR/repair-tests.sh ${REPAIR_SH_ARGS[@]} --test disable"
  "sleep $SLEEPTIME"
  "$BIN ${COMMON_ARGS[@]} configure-gossip-packet-flood --flood-strategy pingCacheOverflow --iteration-delay-us 1000000 --packets-per-peer-per-iteration 10000"
  "sleep $RUNTIME"
  "$BIN ${COMMON_ARGS[@]} configure-gossip-packet-flood"
  "sleep $SLEEPTIME"
  "$BIN ${COMMON_ARGS[@]} configure-replay-stage-attack --selected-attack transferRandom"
  "sleep $RUNTIME"
  "$BIN ${COMMON_ARGS[@]} configure-replay-stage-attack"
  "sleep $SLEEPTIME"
  "$BIN ${COMMON_ARGS[@]} configure-replay-stage-attack --selected-attack createNonceAccounts"
  "sleep $RUNTIME"
  "$BIN ${COMMON_ARGS[@]} configure-replay-stage-attack"
  "sleep $SLEEPTIME"
  "$BIN ${COMMON_ARGS[@]} configure-replay-stage-attack --selected-attack allocateRandomLarge"
  "sleep $RUNTIME"
  "$BIN ${COMMON_ARGS[@]} configure-replay-stage-attack"
  "sleep $SLEEPTIME"
  "$BIN ${COMMON_ARGS[@]} configure-replay-stage-attack --selected-attack allocateRandomSmall"
  "sleep $RUNTIME"
  "$BIN ${COMMON_ARGS[@]} configure-replay-stage-attack"
  "sleep $SLEEPTIME"
  "$BIN ${COMMON_ARGS[@]} configure-replay-stage-attack --selected-attack chainTransactions"
  "sleep $RUNTIME"
  "$BIN ${COMMON_ARGS[@]} configure-replay-stage-attack"
  "sleep $SLEEPTIME"
  "$BIN ${COMMON_ARGS[@]} configure-send-duplicate-blocks --turbine-send-delay-ms 1600"
  "sleep $RUNTIME"
