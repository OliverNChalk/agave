...
impl<'a, M> TransactionPtrBatch<'a, M> {
    ...
    const TX_BATCH_PER_MESSAGE: usize = Self::TX_REGION_SIZE + std::mem::size_of::<M>();
    #[allow(dead_code, reason = "Invariant assertion")]
    const TX_BATCH_SIZE_ASSERT: () =
        assert!(Self::TX_BATCH_PER_MESSAGE * MAX_TRANSACTIONS_PER_MESSAGE < 4096);
    const TX_BATCH_META_OFFSET: usize = Self::TX_REGION_SIZE * MAX_TRANSACTIONS_PER_MESSAGE;
    ...
    pub unsafe fn from_sharable_transaction_batch_region(
        ...
        let meta_ptr = base.byte_add(Self::TX_BATCH_META_OFFSET).cast();
