#![allow(clippy::arithmetic_side_effects)] // required for crossbeam_channel::select!
use {
    crate::ShredDosError,
    crossbeam_channel::{Receiver, RecvError},
    log::{debug, error, info},
    solana_metrics::datapoint_info,
    solana_net_utils::{bind_to_unspecified, sockets::SocketConfiguration},
    solana_perf::{
...
pub fn start_forwarder_threads(
    ...
    solana_net_utils::sockets::multi_bind_in_range_with_config(
        src_addr,
    ...
    .flat_map(|(thread_id, incoming_shred_socket)| {
        ...
        let listen_thread = streamer::receiver(
            format!("solFrwdListen{thread_id:02}"),
            Arc::new(incoming_shred_socket),
            exit.clone(),
            packet_sender,
            recycler.clone(),
            Arc::new(StreamerReceiveStats::new("shred_dos-listen_thread")),
            Some(Duration::ZERO), // do not coalesce since batching consumes more cpu cycles and adds latency.
            false,                // do not pin memory to use minimal RAM
            None,
            // We want forwarding, marking packets as ones that come from stacked nodes enables
            // forwarding.
            true,
        );
        ...
        let send_thread = Builder::new()
            ...
            .spawn(move || {
                let send_socket =
                    bind_to_unspecified().expect("to bind to udp port for forwarding");
...
fn recv_from_channel_and_send_multiple_dest(
    ...
    let num_deduped =
        dedup_packets_and_count_discards(&deduper.read().unwrap(), &mut packet_batch_vec);
    ...
    let packets_len = packets_with_dest.len();
    match batch_send(send_socket, packets_with_dest) {
        Ok(_) => {
            metrics
                .agg_success_forward
                .fetch_add(packets_len as u64, Ordering::Relaxed);
        ...
        Err(SendPktsError::IoError(err, num_failed)) => {
            metrics
                ...
                .fetch_add(packets_len as u64, Ordering::Relaxed);
            error!(
                "Failed to send batch of size {packets_len}. {num_failed} packets failed. Error: \
                 {err}"
            );
        }
    }
...
mod tests {
    use {
        ...
        solana_perf::packet::{Meta, Packet, PacketBatch, PinnedPacketBatch},
        std::{
    ...
    #[test]
    fn test_shred_forwarding() {
        let packet_batch = PacketBatch::from(PinnedPacketBatch::new(vec![
            Packet::new(
                [1; PACKET_DATA_SIZE],
                Meta {
        ...
        let test_listeners = dest_socketaddrs
            ...
            .map(|socketaddr| {
                (
                    bind_to(socketaddr.ip(), socketaddr.port()).unwrap(),
                    *socketaddr,
        ...
        let udp_sender = bind_to(IpAddr::V4(Ipv4Addr::LOCALHOST), 10000).unwrap();
